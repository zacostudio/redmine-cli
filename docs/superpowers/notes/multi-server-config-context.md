...
`8e03f7e` 커밋 단위로 다시 돌렸더니 14건이 나왔는데, 그 중 12건은 이미 1차 리뷰 대응(0690f07,
83353df)에서 고친 것이었다. HEAD 에서 직접 실행해 확인한 결과 실제로 남아 있던 것은 하나다.

**`config server add` 의 기본 서버 승격 조건.** `default_server.is_none()` 이면 승격시키고
있었다. 손으로 서버 둘을 적고 기본값을 안 정해 둔 설정에 세 번째를 추가하면, 원래는 "기본이
없다" 는 에러가 나야 할 상황에서 조용히 새 서버가 기본이 된다. 조건을 `servers.len() == 1` 로
바꿨다. add 출력에도 현재 default_server 를 실어서, 어디로 호출이 나갈지 매번 보이게 했다.

**남긴 것.** stdin 이 열린 채 EOF 가 오지 않으면 멈추는 문제는 stdin 을 읽는 모든 CLI 의 성질이고
